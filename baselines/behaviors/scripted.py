...
def get_scripted_action(obs: jnp.ndarray, behavior_name: str,
    ...
    if behavior_name == "noop":
        ...
        return jax.random.randint(rng_key, (), 0, 5)
    elif behavior_name == "seek":
        # Move toward opponent using JAX-compatible operations
        sx, sy = obs[0], obs[1]
        ox, oy = obs[2], obs[3]
        dx, dy = ox - sx, oy - sy
        
        # Use JAX-compatible conditional operations
        x_dominant = jnp.abs(dx) > jnp.abs(dy)
        action = jnp.where(x_dominant,
                          jnp.where(dx > 0, 2, 1),  # right if dx > 0, left otherwise
                          jnp.where(dy > 0, 4, 3))  # up if dy > 0, down otherwise
        return action
    elif behavior_name == "guardian":
        # Move toward center using JAX-compatible operations
        sx, sy = obs[0], obs[1]
        self_dist = jnp.sqrt(sx ** 2 + sy ** 2)
        SAFE_RAD = 0.15
        
        # Use JAX-compatible conditional operations
        needs_correction = self_dist > SAFE_RAD * 0.9
        x_dominant = jnp.abs(sx) > jnp.abs(sy)
        correction_action = jnp.where(x_dominant,
                                     jnp.where(sx > 0, 1, 2),  # left if right, right if left
                                     jnp.where(sy > 0, 3, 4))  # down if up, up if down
        
        action = jnp.where(needs_correction, correction_action, 0)
        return action
        ...
        # Use JAX-compatible conditional operations
        # Outer bound: move inward
        outer_condition = self_dist > OUTER_BOUND
        x_dominant = jnp.abs(sx) > jnp.abs(sy)
        outer_action = jnp.where(x_dominant,
                                jnp.where(sx > 0, 1, 2),  # left if right, right if left
                                jnp.where(sy > 0, 3, 4))  # down if up, up if down
        
        # Inner bound: move outward  
        inner_condition = self_dist < INNER_BOUND
        inner_action = jnp.where(jnp.abs(sx) < jnp.abs(sy),
                                jnp.where(sx >= 0, 2, 1),  # right if right, left if left
                                jnp.where(sy >= 0, 4, 3))  # up if up, down if down
        
        # Default: random tangential movement
        default_action = jax.random.randint(rng_key, (), 1, 5)
        
        # Chain the conditions
        action = jnp.where(outer_condition, outer_action,
                          jnp.where(inner_condition, inner_action, default_action))
