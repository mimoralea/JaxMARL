...
class SPPPOCheckpointManager:
    ...
    def __init__(
        ...
        """
        self.checkpoint_dir = Path(checkpoint_dir).resolve()  # Ensure absolute path
    ...
    def load_latest_checkpoint(self) -> Optional[Tuple[TrainState, Dict[str, Any]]]:
...
class SPPPOCheckpointCallback:
    ...
    def __call__(self, step: int, train_state: TrainState, is_final: bool = False) -> Optional[str]:
        ...
        if should_save and step not in self.saved_steps:
            metadata = {
                'training_step': step,
                'is_final': int(is_final),  # Convert bool to int for StandardCheckpointer
    ...
    def save_final_checkpoint(self, train_state: TrainState, step: int) -> Optional[str]:
