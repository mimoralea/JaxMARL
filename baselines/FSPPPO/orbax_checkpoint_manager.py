...
class FSPPPOCheckpointManager:
    ...
    def __init__(self, base_dir: str = "checkpoints", algorithm: str = "fspppo"):
    ...
    def get_checkpoint_dir(self, run_id: str, agent_id: str, update_step: int) -> Path:
        """Get directory path for a checkpoint."""
        # Convert update_step to int to handle JAX array values
        step_int = int(update_step)
        checkpoint_dir = self.base_dir / self.algorithm / run_id / agent_id / f"step_{step_int:06d}"
        return checkpoint_dir.resolve()  # Convert to absolute path
    
    def save_checkpoint(self, params: Any, update_step: int, run_id: Optional[str] = None, 
                       agent_id: str = "main_agent") -> str:
        """
        ...
        self._update_agent_metadata(run_id, agent_id, step_int, str(checkpoint_dir))
        
        return str(checkpoint_dir)
    
    def load_checkpoint(self, checkpoint_dir: str, abstract_params: Any) -> Any:
        """
    ...
    def _update_agent_metadata(self, run_id: str, agent_id: str, update_step: int, checkpoint_dir: str):
        ...
        agent_metadata["checkpoints"] = [
            cp for cp in agent_metadata["checkpoints"] 
    ...
    def get_available_runs(self) -> List[str]:
        ...
        runs = [d.name for d in algorithm_dir.iterdir() 
                if d.is_dir() and d.name.startswith("run_")]
        return sorted(runs)
    
    def get_agent_checkpoints(self, run_id: str, agent_id: str = "main_agent") -> List[Dict]:
    ...
    def cleanup_old_checkpoints(self, run_id: str, agent_id: str = "main_agent", 
                               max_checkpoints: int = 10) -> int:
        """
    ...
    def get_latest_checkpoint(self, run_id: str, agent_id: str = "main_agent") -> Optional[Dict]:
        """Get the most recent checkpoint for an agent."""
        checkpoints = self.get_agent_checkpoints(run_id, agent_id)
        return checkpoints[-1] if checkpoints else None
    
    def get_checkpoint_for_step(self, run_id: str, agent_id: str, update_step: int) -> Optional[Dict]:
    ...
    def print_checkpoint_summary(self, run_id: str, agent_id: str = "main_agent"):
