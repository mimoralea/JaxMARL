...
Based on BR implementation - trains one agent against fixed learned opponents
"""
        ...
        def _update_step(runner_state, update_idx):
            ...
            def _env_step(runner_state, unused):
                ...
                for i in range(env.num_agents):
                    pi_i, v_i = network.apply(
                        train_state.params[i], obs_split[i]
                    )
            ...
            def _update_epoch(update_state, unused):
                def _update_minbatch(train_state, batch_info):
                    ...
                    for agent_idx in range(env.num_agents):
                        ...
                        tb_agent = jax.tree_util.tree_map(
                            lambda x: x[sel], traj_batch
                        )
                    ...
                    (loss0, aux0), grads0 = jax.value_and_grad(
                        _loss_single, has_aux=True
                    )(
                        train_state.params[0],
                        mb_split[0][0],
                        mb_split[0][1],
                        mb_split[0][2],
                    )
                    (loss1, aux1), grads1 = jax.value_and_grad(
                        _loss_single, has_aux=True
                    )(
                        train_state.params[1],
                        mb_split[1][0],
                        mb_split[1][1],
                        mb_split[1][2],
                    )
                    ...
                    new_params = optax.apply_updates(
                        train_state.params, updates_tuple
                    )
...
def get_rollout(
    ...
    # BR has two agents, so train_state.params is (agent_0_params, agent_1_params)
