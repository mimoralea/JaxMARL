"""Orbax-based checkpoint management for IPPO.

Provides unified checkpoint saving/loading for IPPO using the same structure
as FSPPPO to enable cross-algorithm evaluation and comparison.

Structure: checkpoints/ippo/run_{run_id}_seed{X}/agent_{0|1}/step_{step}/
"""
import os
import time
...
class IPPOCheckpointManager:
    """Checkpoint manager for IPPO using Orbax."""
    ...
    def save_checkpoint(
        ...
        for agent_name in self.agent_names:
            if agent_name not in train_states:
                raise ValueError(f"Missing train_state for agent: {agent_name}")
            ...
            step_int = int(step)
            checkpoint_path = self.agent_dirs[agent_name] / f"step_{step_int:06d}"
    ...
    def load_checkpoint(self, step: int) -> Tuple[Dict[str, TrainState], Dict[str, Any]]:
    ...
    def load_latest_checkpoint(self) -> Optional[Tuple[Dict[str, TrainState], Dict[str, Any]]]:
    ...
    def cleanup_old_checkpoints(self) -> Dict[str, int]:
        ...
        for agent_name in self.agent_names:
            ...
            for step in to_remove:
                try:
                    checkpoint_path = self.agent_dirs[agent_name] / f"step_{step:06d}"
                    if checkpoint_path.exists():
                        import shutil
                        shutil.rmtree(checkpoint_path)
                        removed_count += 1
                except Exception as e:
                    print(f"Warning: Failed to remove checkpoint {step} for {agent_name}: {e}")
...
def create_ippo_checkpoint_manager(
    run_id: str,
    seed: int,
    base_dir: str = "checkpoints",
    max_to_keep: Optional[int] = 10,
    agent_names: List[str] = None
) -> IPPOCheckpointManager:
    """Create a checkpoint manager for IPPO with standard directory structure.
    ...
    """
    checkpoint_dir = os.path.join(base_dir, "ippo", f"{run_id}_seed{seed}")
...
class IPPOCheckpointCallback:
    """Callback for saving checkpoints during IPPO training."""
    
    def __init__(
        self,
        checkpoint_manager: IPPOCheckpointManager,
    ...
    def __call__(
        ...
        """
        should_save = (
            (step % self.save_frequency == 0 and step > 0) or
            (is_final and self.save_at_end) or
            step == 0  # Always save initial checkpoint
        )
    ...
    def save_final_checkpoint(
        ...
        """Save final checkpoint for IPPO training.
        ...
        return self.__call__(step=step, train_states=train_states, is_final=True)


# Utility functions for backward compatibility
def load_ippo_pickle_checkpoint(pickle_path: str) -> Any:
    """Load legacy IPPO pickle checkpoint.
...
def convert_pickle_to_orbax(
    ...
    output_manager: IPPOCheckpointManager,
    step: int = 0,
    ...
    for agent_name, pickle_path in pickle_paths.items():
        params = load_ippo_pickle_checkpoint(pickle_path)
